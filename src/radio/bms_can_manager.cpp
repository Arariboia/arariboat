...
void BMSCANManager::initialize_can_handlers() {
    ...
    // --- Software-level exclusion for polling request messages ---
    // By providing an empty handler, we ensure that if these polling messages
    // are received, they are safely and explicitly ignored by the application logic.
    // This is useful if hardware filters are configured to allow these IDs through,
    // but we don't want to process them here.
    _can_handlers[BMS_VOLTAGE_ID_POLL] = [](const twai_message_t& message) {
        return;
    };
    _can_handlers[BMS_CHARGE_DISCHARGE_ID_POLL] = [](const twai_message_t& message) {
        return;
    };
    _can_handlers[BMS_STATUS_ID_POLL] = [](const twai_message_t& message) {
        return;
    };
    _can_handlers[BMS_CELL_VOLTAGE_ID_POLL] = [](const twai_message_t& message) {
        return;
    };
    _can_handlers[BMS_TEMPERATURE_ID_POLL] = [](const twai_message_t& message) {
        return;
    };
    _can_handlers[BMS_FAILURE_ID_POLL] = [](const twai_message_t& message) {
        return;
    };
}
...
void BMSCANManager::check_and_publish() {
    if (_received_flags == ALL_FLAGS && _data_queue) {
        message_t msg;
        msg.source = DATA_SOURCE_BMS;
        msg.payload.bms = _bms_data;
