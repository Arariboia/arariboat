...
class ModbusMaster
{
  public:
    ...
    bool getSolarVoltage(float& voltage);
    bool getSolarCurrent(float& voltage);
  ...
  private:
    ...
    uint8_t _u8RawResponseBuffer[ku8MaxBufferSize];             ///< buffer to store raw Modbus response
    uint8_t _u8RawResponseBufferLength;                            ///< length of raw Modbus response
    ...
    public:
    uint8_t  sniffer();
    uint8_t transactionSniffer(uint8_t u8MBFunction);
    private:
