...
uint8_t ModbusMaster::sniffer() {
  	return transactionSniffer(ku8MBReadInputRegisters);
}
...
/*
frames
01 04 31 00 00 11 3E FA 01 04 22 00 00 00 00 00 00 00 00 04 E2 00 00 03 04 31 09 00 03 04 31 09 00 03 04 32 02 00 03 04 32 02 00 01 04 32 00 00 03 BE B3 01 04 06 00 00 00 01 00 01 F0 93 
*/

//Dummy class to mock sniffed bytes on the bus
class FakeSerial : public Stream {
  public:
    ...
    private:
      const char *frames = "01 04 31 00 00 11 3E FA 01 04 22 00 00 00 00 00 00 00 00 04 E2 00 00 03 04 31 09 00 03 04 31 09 00 03 04 32 02 00 03 04 32 02 00 01 04 32 00 00 03 BE B3 01 04 06 00 00 00 01 00 01 F0 93";
      int frameIndex = 0;
      int read() {
        if (frameIndex >= strlen(frames)) {
          frameIndex = 0;
        }
...
bool ModbusMaster::getBatteryVoltage(float& voltage) {
    if (_u8RawResponseBuffer[0] != _u8MBSlave || 
        _u8RawResponseBuffer[1] != 0x04       || 
        _u8RawResponseBuffer[2] != 0x33) {
          return false;
    }

	voltage = (_u8RawResponseBuffer[52] << 8 | _u8RawResponseBuffer[53]) / 100.0f ;
	return true;
}

bool ModbusMaster::getSolarVoltage(float& voltage) {
	if (_u8RawResponseBuffer[0] != _u8MBSlave || 
        _u8RawResponseBuffer[1] != 0x04       || 
        _u8RawResponseBuffer[2] != 0x33) {
		return false;
	...
	voltage = (_u8RawResponseBuffer[44] << 8 | _u8RawResponseBuffer[45]) / 100.0f ;
	return true;
}

bool ModbusMaster::getSolarCurrent(float& voltage) {
	...
	voltage = (_u8RawResponseBuffer[46] << 8 | _u8RawResponseBuffer[47]) / 100.0f ;
	return true;
}


/**
Modbus transaction engine only for reading data from slave.
This is meant to be a sniffer for Modbus data on the bus.
Sequence:
  - wait for/retrieve response
  - evaluate/disassemble response
  - return status (success/exception)
...
@return 0 on success; exception number on failure
*/
uint8_t ModbusMaster::transactionSniffer(uint8_t u8MBFunction) {
	...
	while (u8BytesLeft && !u8MBStatus) {
		...
		if (u8ModbusADUSize == 5) {
			...
			switch(u8ModbusADU[1]) {
				case ku8MBReadCoils:
				case ku8MBReadDiscreteInputs:
				case ku8MBReadInputRegisters: //0x04
				case ku8MBReadHoldingRegisters:
				case ku8MBReadWriteMultipleRegisters:
				u8BytesLeft = u8ModbusADU[2];
				break;
	...
	if (!u8MBStatus) {
		...
		case ku8MBReadHoldingRegisters:
		case ku8MBReadWriteMultipleRegisters:
			memcpy(_u8RawResponseBuffer, u8ModbusADU, u8ModbusADUSize);
			_u8RawResponseBufferLength = u8ModbusADUSize;
			break;
		}
	}
